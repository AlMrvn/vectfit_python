...
def vectfit_auto_rescale(f, s, option=False, **kwargs):
    s_scale = abs(s[-1])
    if option:
        f_scale = max(abs(f))
    else:
    ...
    residues = np.array([[res * f_scale * s_scale for res in residues]
                         for residues in residues_s])
    print('shape de residues : {0} '.format(np.shape(residues)))
    d = d_s * f_scale
...
if __name__ == '__main__':
    # testing the example similar of [1]
